  ...
  <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑车主' : '新增车主'" width="90%" :style="{ maxWidth: '520px' }">
    <el-form ref="formRef" :model="form" :rules="rules" label-width="110px">
      <el-form-item label="姓名" prop="name">
        <el-input v-model="form.name" placeholder="车主姓名" />
      </el-form-item>
      <el-form-item label="身份" prop="role">
        <AppSelect v-model="form.role" :options="OWNER_ROLE_OPTIONS" style="width: 100%" />
      </el-form-item>
      <el-form-item prop="company_fee_rate">
        <template #label>公司费率(%)<FieldTip content="从「平台结算金额」里公司抽成的比例。台账里捷途是 15%、雅阁是 10%，自营车填 0。改费率只影响之后生成的结算行，已生成的不会被改写。" /></template>
      ...
      <el-form-item>
        <template #label>往来期初<FieldTip content="正数 = 公司应付此人。只填开始用系统时的余额，不要填历史累计" /></template>
