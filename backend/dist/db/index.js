...
function runMigrations(db) {
    try {
      ...
      `);
        ...
        // 检查订单来源表是否有 color 字段
        const sourceColumns = db.pragma('table_info(order_sources)');
        ...
        const sourceCount = db.prepare('SELECT COUNT(*) as count FROM order_sources').get();
      ...
      `);
            ...
            const inspectionColumns = db.pragma('table_info(inspections)');
        ...
        // 检查违章表字段
        const violationColumns = db.pragma('table_info(violations)');
        ...
        const insuranceColumns = db.pragma('table_info(insurance)');
        ...
        const maintenanceColumns = db.pragma('table_info(maintenance)');
        ...
        // 检查vehicles表字段
        const vehicleColumns = db.pragma('table_info(vehicles)');
        ...
        // 检查customers表字段
        const customerColumns = db.pragma('table_info(customers)');
      ...
      `);
        ...
        const rentedCount = db.prepare("SELECT COUNT(*) as count FROM vehicles WHERE status = 'rented'").get();
