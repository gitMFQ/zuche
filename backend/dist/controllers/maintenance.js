...
// 获取保养列表
export function getMaintenanceList(req, res) {
    try {
...
// 获取保养统计
export function getMaintenanceStats(req, res) {
    try {
        // 本月保养数量
...
export function createMaintenance(req, res) {
    try {
        const { vehicle_id, plate_number, type, maintenance_date, cost, mileage, garage, next_maintenance_date, next_maintenance_mileage, images, remarks, status } = req.body;
        ...
        execute(`INSERT INTO maintenance (
        ...
        // 更新车辆的里程和上次保养日期
        if (mileage) {
            execute('UPDATE vehicles SET mileage = ?, last_maintenance = ?, updated_at = ? WHERE id = ?', [mileage, maintenance_date, currentTime, vehicle_id]);
        }
        res.json({
            success: true,
...
export function updateMaintenance(req, res) {
    try {
        const { id } = req.params;
        const { vehicle_id, type, maintenance_date, cost, mileage, garage, next_maintenance_date, next_maintenance_mileage, images, remarks, status } = req.body;
        ...
        // 处理图片
        const imagesStr = images && images.length > 0 ? JSON.stringify(images) : null;
        execute(`UPDATE maintenance SET 
