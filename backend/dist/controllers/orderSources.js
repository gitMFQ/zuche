...
// 获取订单来源列表
export function getOrderSources(req, res) {
    try {
...
// 获取单个订单来源
export function getOrderSource(req, res) {
    try {
        const { id } = req.params;
...
// 创建订单来源
export function createOrderSource(req, res) {
    try {
        ...
        execute('INSERT INTO order_sources (id, name, commission_rate, color, remarks, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 1, ?, ?)', [id, name, commission_rate || 0, color || '#409EFF', remarks || null, currentTime, currentTime]);
        // 记录操作日志
...
// 更新订单来源
export function updateOrderSource(req, res) {
    try {
        const { id } = req.params;
        const { name, commission_rate, color, remarks } = req.body;
        const source = queryOne('SELECT * FROM order_sources WHERE id = ?', [id]);
        ...
        const currentTime = now();
        execute('UPDATE order_sources SET name = ?, commission_rate = ?, color = ?, remarks = ?, updated_at = ? WHERE id = ?', [name || source.name, commission_rate ?? source.commission_rate, color || source.color, remarks ?? source.remarks, currentTime, id]);
        // 记录操作日志
...
// 删除订单来源
export function deleteOrderSource(req, res) {
    try {
        const { id } = req.params;
        const source = queryOne('SELECT * FROM order_sources WHERE id = ?', [id]);
        if (!source) {
