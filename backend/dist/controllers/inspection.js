...
// 获取车辆年检状态列表（显示所有车辆）
export function getInspectionList(req, res) {
    try {
    ...
    `;
        ...
        // 按状态筛选（在获取数据后筛选）
        if (status) {
            result.data = result.data.filter((item) => item.inspection_status === status);
...
// 获取年检统计
export function getInspectionStats(req, res) {
    try {
        const today = new Date().toISOString().split('T')[0];
...
export function createInspection(req, res) {
    try {
        ...
        if (existing) {
            // 更新现有记录
            execute(`UPDATE inspections SET expiry_date = ?, certificate_image = ?, remarks = ?, updated_at = ? WHERE vehicle_id = ?`, [expiry_date, certificate_image || null, remarks || null, currentTime, vehicle_id]);
            res.json({ success: true, message: '年检证更新成功' });
        }
        else {
            // 创建新记录
            const id = generateId();
            execute(`INSERT INTO inspections (id, vehicle_id, plate_number, expiry_date, certificate_image, remarks, status, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, 'valid', ?, ?)`, [id, vehicle_id, vehicle.plate_number, expiry_date, certificate_image || null, remarks || null, currentTime, currentTime]);
...
// 更新年检记录（通过车辆ID）
export function updateInspection(req, res) {
    try {
        ...
        if (!inspection) {
         ...
         VALUES (?, ?, ?, ?, ?, ?, 'valid', ?, ?)`, [id, vehicle_id, vehicle.plate_number, expiry_date, certificate_image || null, remarks || null, currentTime, currentTime]);
            ...
            // 更新现有记录
            execute(`UPDATE inspections SET expiry_date = ?, certificate_image = ?, remarks = ?, updated_at = ? WHERE vehicle_id = ?`, [expiry_date, certificate_image || null, remarks || null, now(), vehicle_id]);
        }
...
// 删除年检记录（通过车辆ID）
export function deleteInspection(req, res) {
    try {
        const { vehicle_id } = req.params;
