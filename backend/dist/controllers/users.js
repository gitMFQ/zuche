...
export function getUser(req, res) {
    try {
        const { id } = req.params;
        const user = queryOne('SELECT id, username, name, role, phone, email, status, created_at FROM users WHERE id = ?', [id]);
        if (!user) {
...
export function createUser(req, res) {
    try {
        ...
        execute('INSERT INTO users (id, username, password, name, role, phone, email, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?)', [id, username, hashedPassword, name, role, phone || null, email || null, currentTime, currentTime]);
        // 记录操作日志
...
export function updateUser(req, res) {
    try {
        ...
        execute('UPDATE users SET name = ?, role = ?, phone = ?, email = ?, status = ?, updated_at = ? WHERE id = ?', [name, role, phone || null, email || null, status, now(), id]);
        // 记录操作日志
