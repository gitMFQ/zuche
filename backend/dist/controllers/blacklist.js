...
// 获取黑名单列表
export function getBlacklist(req, res) {
    try {
...
// 检查是否在黑名单（根据手机号或身份证）
export function checkBlacklist(req, res) {
    try {
...
// 添加到黑名单
export function addToBlacklist(req, res) {
    try {
        ...
        execute(`INSERT INTO blacklist (id, customer_id, name, phone, id_card, reason, order_id, operator_id, operator_name, status, created_at, updated_at) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?)`, [id, customer_id || null, name, phone, id_card || null, reason, order_id || null, operatorId, operatorName, currentTime, currentTime]);
        // 记录操作日志
...
// 从黑名单移除
export function removeFromBlacklist(req, res) {
    try {
        const { id } = req.params;
...
// 获取黑名单详情
export function getBlacklistDetail(req, res) {
    try {
        const { id } = req.params;
        const record = queryOne('SELECT * FROM blacklist WHERE id = ?', [id]);
        if (!record) {
