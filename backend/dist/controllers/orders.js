...
export function getOrders(req, res) {
    try {
        const { page = 1, pageSize = 10, keyword = '', status = '', customer_id = '', vehicle_id = '', start_date_from = '', start_date_to = '', end_date_from = '', end_date_to = '', source_id = '', vehicle_model = '', plate_number = '', order_by = '' } = req.query;
        let sql = `
      SELECT o.*,
...
export function createOrder(req, res) {
    try {
        const { 
        // 客户信息（直接填写）
        customer_name, customer_phone, customer_id_card, customer_license, id_card_images, license_images, 
        // 车辆和租期信息
        vehicle_id, start_date, end_date, daily_rate, deposit, total_amount, remarks, 
        // 订单来源
        source_id, 
        ...
        // 预付相关
        has_prepay, prepay_amount, prepay_method, prepay_type } = req.body;
        // 验证必填字段（日租金和总租金至少填一个）
    ...
    `, [vehicle_id, start_date, start_date, end_date, end_date, start_date, end_date]);
        ...
        if (customer) {
            ...
            execute('UPDATE customers SET name = ?, id_card = ?, license_number = ?, id_card_images = ?, license_images = ?, updated_at = ? WHERE id = ?', [
                customer_name,
            ...
            execute('INSERT INTO customers (id, name, phone, id_card, license_number, id_card_images, license_images, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?)', [
                customerId,
                customer_name,
                customer_phone,
                customer_id_card || null,
...
export function updateOrderStatus(req, res) {
    try {
        ...
        const updateParams = [status, actual_end_date || null, totalAmount, remarks || order.remarks, currentTime];
        // 取车时记录取车里程和图片
        if (status === 'active') {
...
export function updateOrder(req, res) {
    try {
        const { id } = req.params;
        const { customer_name, customer_phone, customer_id_card, customer_license, id_card_images, license_images, vehicle_id, source_id, start_date, end_date, daily_rate, total_amount, deposit, remarks, service_type, deposit_waived, deposit_waived_expiry, contract_number, pickup_location, return_location } = req.body;
        const order = queryOne('SELECT * FROM orders WHERE id = ?', [id]);
        if (!order) {
            res.status(404).json({ success: false, message: '订单不存在' });
            return;
        ...
        if (customer_name || customer_phone || customer_id_card || customer_license || id_card_images || license_images) {
            ...
            // 更新客户信息
            execute('UPDATE customers SET name = ?, phone = ?, id_card = ?, license_number = ?, id_card_images = ?, license_images = ?, updated_at = ? WHERE id = ?', [
                customer_name,
                customer_phone,
                customer_id_card || null,
                customer_license || null,
...
// 续租订单
export function extendOrder(req, res) {
    try {
        const { id } = req.params;
        ...
        // 更新订单
        execute('UPDATE orders SET end_date = ?, total_amount = ?, net_amount = ?, paid_amount = ?, updated_at = ? WHERE id = ?', [new_end_date, newTotalAmount, newNetAmount, newPaidAmount, currentTime, id]);
        // 如果有支付，添加支付记录
        if (has_payment && payment_amount > 0 && payment_method) {
            const paymentId = generateId();
            execute('INSERT INTO payments (id, order_id, amount, payment_method, payment_type, remarks, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)', [paymentId, id, payment_amount, payment_method, 'rent', '续租支付', currentTime]);
        }
        // 记录操作日志
        const vehicle = queryOne('SELECT plate_number FROM vehicles WHERE id = ?', [order.vehicle_id]);
...
export function addPayment(req, res) {
    try {
        ...
        const currentTime = now();
        // 添加支付记录
        execute('INSERT INTO payments (id, order_id, amount, payment_method, payment_type, remarks, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)', [paymentId, id, amount, payment_method, payment_type, remarks || null, currentTime]);
        // 更新订单已支付金额
...
export function cancelOrder(req, res) {
    try {
        ...
        const currentTime = now();
        // 更新订单状态
        execute("UPDATE orders SET status = 'cancelled', remarks = ?, updated_at = ? WHERE id = ?", [remarks || order.remarks, currentTime, id]);
        // 记录操作日志
        const vehicle = queryOne('SELECT plate_number FROM vehicles WHERE id = ?', [order.vehicle_id]);
