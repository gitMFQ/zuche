...
import { queryOne, execute, now, logAction } from '../utils/helpers.js';
...
export async function login(req, res) {
    try {
        ...
        // 支持用户名、姓名、手机号、邮箱登录
        const user = queryOne('SELECT * FROM users WHERE (username = ? OR name = ? OR phone = ? OR email = ?) AND status = 1', [username, username, username, username]);
        if (!user) {
...
export function changePassword(req, res) {
    try {
        ...
        execute('UPDATE users SET password = ?, updated_at = ? WHERE id = ?', [hashedPassword, now(), userId]);
