import { queryOne, execute, generateId, now, queryWithPagination, logAction } from '../utils/helpers.js';
// 违章状态映射
const STATUS_MAP = {
    pending: '待处理',
    processing: '处理中',
...
// 获取违章列表
export function getViolations(req, res) {
    try {
...
// 获取单个违章
export function getViolation(req, res) {
    try {
        const { id } = req.params;
...
// 创建违章记录
export function createViolation(req, res) {
    try {
        const { order_id, vehicle_id, customer_id, customer_name, customer_phone, plate_number, violation_type, violation_date, location, fine_amount, penalty_points, penalty_fee, images, remarks } = req.body;
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?)`, [id, order_id || null, vehicle_id, customer_id || null, customer_name, customer_phone || null, plate_number,
...
// 更新违章记录
export function updateViolation(req, res) {
    try {
        const { id } = req.params;
        const { violation_type, violation_date, location, fine_amount, penalty_points, penalty_fee, images, remarks } = req.body;
        const violation = queryOne('SELECT * FROM violations WHERE id = ?', [id]);
        if (!violation) {
            res.status(404).json({ success: false, message: '违章记录不存在' });
            return;
        ...
        const currentTime = now();
        execute(`UPDATE violations SET 
        violation_type = ?, violation_date = ?, location = ?, 
        fine_amount = ?, penalty_points = ?, penalty_fee = ?, images = ?, remarks = ?, updated_at = ? 
       WHERE id = ?`, [violation_type, violation_date, location || null, fine_amount || 0, penalty_points || 0, penalty_fee || 0, imagesJson, remarks || null, currentTime, id]);
...
// 处理违章（标记已处理）
export function handleViolation(req, res) {
    try {
        const { id } = req.params;
        ...
        const currentTime = now();
        execute(`UPDATE violations SET status = ?, handle_date = ?, handle_remarks = ?, handle_type = ?, license_deposit = ?, updated_at = ? WHERE id = ?`, [status, currentTime, handle_remarks || null, handle_type || 'store', license_deposit || 0, currentTime, id]);
        // 记录操作日志
...
// 收取违章费用（违约金、罚款）
export function collectFee(req, res) {
    try {
        const { id } = req.params;
       ...
       WHERE id = ?`, [collected_penalty || 0, collected_fine || 0, fee_remarks || null, currentTime, id]);
...
// 删除违章记录
export function deleteViolation(req, res) {
    try {
        const { id } = req.params;
        const violation = queryOne('SELECT * FROM violations WHERE id = ?', [id]);
        if (!violation) {
...
// 获取违章统计
export function getViolationStats(req, res) {
    try {
