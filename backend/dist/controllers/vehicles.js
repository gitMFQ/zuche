...
export function getAvailableVehicles(req, res) {
    try {
...
export function createVehicle(req, res) {
    try {
        ...
        execute(`INSERT INTO vehicles (id, plate_number, brand, model, color, year, seats, daily_rate, deposit, mileage, vin, engine_number, license_image, registration_image, is_new_energy, remarks, status, created_at, updated_at) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'available', ?, ?)`, [id, plate_number, brand, model, color || null, year || null, seats || 5, daily_rate || 0, deposit || 0, mileage || 0, vin || null, engine_number || null, license_image || null, registration_image || null, is_new_energy ? 1 : 0, remarks || null, currentTime, currentTime]);
        // 记录操作日志
...
export function updateVehicle(req, res) {
    try {
        ...
        execute(`UPDATE vehicles SET plate_number = ?, brand = ?, model = ?, color = ?, year = ?, seats = ?, daily_rate = ?, deposit = ?, status = ?, mileage = ?, last_maintenance = ?, vin = ?, engine_number = ?, license_image = ?, registration_image = ?, is_new_energy = ?, remarks = ?, updated_at = ? WHERE id = ?`, [plate_number, brand, model, color || null, year || null, seats || 5, daily_rate || 0, deposit || 0, status, mileage || 0, last_maintenance || null, vin || null, engine_number || null, license_image || null, registration_image || null, is_new_energy ? 1 : 0, remarks || null, now(), id]);
        // 记录操作日志
...
export function deleteVehicle(req, res) {
    try {
        const { id } = req.params;
        // 检查是否有未完成的订单
        const activeOrders = queryOne("SELECT COUNT(*) as count FROM orders WHERE vehicle_id = ? AND status IN ('pending', 'active')", [id]);
...
// 获取车辆品牌列表
export function getVehicleBrands(req, res) {
    try {
