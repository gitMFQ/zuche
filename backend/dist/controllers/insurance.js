...
// 获取保险列表
export function getInsuranceList(req, res) {
    try {
...
// 获取保险统计
export function getInsuranceStats(req, res) {
    try {
        const today = new Date().toISOString().split('T')[0];
...
// 获取单个保险记录
export function getInsurance(req, res) {
    try {
        const { id } = req.params;
...
// 创建保险记录
export function createInsurance(req, res) {
    try {
        const { vehicle_id, plate_number, insurance_type, insurance_types, insurance_company, policy_number, start_date, end_date, premium, coverage_amount, beneficiary, documents, remarks } = req.body;
        // 支持数组或单个值
...
// 更新保险记录
export function updateInsurance(req, res) {
    try {
        const { id } = req.params;
        const { vehicle_id, insurance_type, insurance_types, insurance_company, policy_number, start_date, end_date, premium, coverage_amount, beneficiary, documents, remarks, status } = req.body;
...
// 删除保险记录
export function deleteInsurance(req, res) {
    try {
        const { id } = req.params;
