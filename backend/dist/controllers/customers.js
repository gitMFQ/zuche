...
export function createCustomer(req, res) {
    try {
        ...
        execute(`INSERT INTO customers (id, name, phone, id_card, license_number, license_expiry, address, remarks, id_card_images, license_images, source_id, source_name, status, created_at, updated_at) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?)`, [id, name, phone, id_card || null, license_number || null, license_expiry || null, address || null, remarks || null,
...
export function updateCustomer(req, res) {
    try {
        ...
        execute(`UPDATE customers SET name = ?, phone = ?, id_card = ?, license_number = ?, license_expiry = ?, address = ?, remarks = ?, status = ?, id_card_images = ?, license_images = ?, source_id = ?, source_name = ?, updated_at = ? WHERE id = ?`, [name, phone, id_card || null, license_number || null, license_expiry || null, address || null, remarks || null, status,
            id_card_images ? JSON.stringify(id_card_images) : null,
            license_images ? JSON.stringify(license_images) : null,
...
export function deleteCustomer(req, res) {
    try {
        const { id } = req.params;
        // 检查是否有未完成的订单
        const activeOrders = queryOne("SELECT COUNT(*) as count FROM orders WHERE customer_id = ? AND status IN ('pending', 'active')", [id]);
...
// 获取常用客户列表
export function getRegularCustomers(req, res) {
    try {
...
// 设置/取消常用客户
export function setRegularCustomer(req, res) {
    try {
        const { id } = req.params;
