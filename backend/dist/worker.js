...
// 分页查询
async function queryWithPagination(db, sql, params = [], page = 1, pageSize = 10) {
...
app.post('/api/auth/login', async (c) => {
    try {
        ...
        const user = await queryOne(c.env.DB, 'SELECT * FROM users WHERE username = ?', [username]);
        if (!user) {
        ...
        // 生成 JWT token
        const token = await generateJWT({ id: user.id, username: user.username, role: user.role }, c.env.JWT_SECRET);
        return c.json({
            success: true,
            data: {
                token,
...
app.get('/api/users', authMiddleware, async (c) => {
    try {
        ...
        const result = await queryWithPagination(c.env.DB, 'SELECT id, username, name, role, created_at FROM users ORDER BY created_at DESC', [], page, pageSize);
...
app.post('/api/users', authMiddleware, async (c) => {
    try {
        ...
        // 检查用户名是否已存在
        const existing = await queryOne(c.env.DB, 'SELECT id FROM users WHERE username = ?', [username]);
        if (existing) {
        ...
        await execute(c.env.DB, 'INSERT INTO users (id, username, password_hash, name, role) VALUES (?, ?, ?, ?, ?)', [id, username, passwordHash, name || username, role || 'staff']);
...
app.post('/api/customers', authMiddleware, async (c) => {
    try {
        ...
        const id = crypto.randomUUID();
        await execute(c.env.DB, 'INSERT INTO customers (id, name, phone, id_card, driving_license, source, remarks) VALUES (?, ?, ?, ?, ?, ?, ?)', [id, name, phone, id_card || null, driving_license || null, source || 'store', remarks || '']);
...
app.post('/api/vehicles', authMiddleware, async (c) => {
    try {
        const body = await c.req.json();
        const { license_plate, brand, model, color, year, seats, vin, engine_number, is_new_energy, status, remarks } = body;
        if (!license_plate || !brand) {
            return c.json({ success: false, message: '车牌号和品牌不能为空' }, 400);
        }
        const id = crypto.randomUUID();
        await execute(c.env.DB, `INSERT INTO vehicles (id, license_plate, brand, model, color, year, seats, 
         vin, engine_number, is_new_energy, status, remarks) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [id, license_plate, brand, model || '', color || '', year || new Date().getFullYear(),
...
app.post('/api/orders', authMiddleware, async (c) => {
    try {
        const body = await c.req.json();
        const { customer_id, vehicle_id, service_type, start_date, end_date, pickup_location, return_location, daily_rate, days, total_amount, deposit, deposit_waived, remarks } = body;
        if (!customer_id || !vehicle_id || !start_date || !end_date) {
            return c.json({ success: false, message: '缺少必要参数' }, 400);
        }
        const id = crypto.randomUUID();
        const orderNo = 'ORD' + Date.now();
        await execute(c.env.DB, `INSERT INTO orders (id, order_no, customer_id, vehicle_id, service_type,
         start_date, end_date, pickup_location, return_location,
         daily_rate, days, total_amount, deposit, deposit_waived, status, remarks)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?)`, [id, orderNo, customer_id, vehicle_id, service_type || 'basic',
            start_date, end_date, pickup_location || '', return_location || '',
...
app.get('/api/dashboard/stats', authMiddleware, async (c) => {
    try {
        ...
        // 今日取车订单数
        const pickupsToday = await queryOne(c.env.DB, "SELECT COUNT(*) as count FROM orders WHERE DATE(start_date) = ? AND status != 'cancelled'", [today]);
        // 今日还车订单数
        const returnsToday = await queryOne(c.env.DB, "SELECT COUNT(*) as count FROM orders WHERE DATE(end_date) = ? AND status != 'cancelled'", [today]);
        // 待处理订单数
        const pendingOrders = await queryOne(c.env.DB, "SELECT COUNT(*) as count FROM orders WHERE status = 'pending'");
        // 可用车辆数
        const availableVehicles = await queryOne(c.env.DB, "SELECT COUNT(*) as count FROM vehicles WHERE status = 'available'");
        return c.json({
            success: true,
            data: {
...
app.get('/api/settings', authMiddleware, async (c) => {
    try {
        const rows = await query(c.env.DB, 'SELECT key, value FROM system_settings');
        const settings = {};
        rows.forEach((row) => {
...
app.get('/api/settings/:key', authMiddleware, async (c) => {
    try {
        ...
        const row = await queryOne(c.env.DB, 'SELECT value FROM system_settings WHERE key = ?', [key]);
...
app.put('/api/settings', authMiddleware, async (c) => {
    try {
        ...
        await execute(c.env.DB, `INSERT OR REPLACE INTO system_settings (key, value, updated_at) VALUES (?, ?, datetime('now'))`, [key, value || '']);
