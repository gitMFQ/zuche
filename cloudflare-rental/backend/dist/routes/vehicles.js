...
app.post('/', authMiddleware, async (c) => {
    try {
        ...
        const id = generateId();
        await c.env.DB.prepare(`INSERT INTO vehicles (id, plate_number, brand, model, color, year, seats, mileage, daily_rate, status, is_new_energy, vin, engine_number, license_image, registration_image, remarks, created_at, updated_at) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'available', ?, ?, ?, ?, ?, ?, ?, ?)`).bind(id, plate_number, brand, model, color || null, year || null, seats || 5, mileage || 0, daily_rate || 0, is_new_energy ? 1 : 0, vin || null, engine_number || null, license_image || null, registration_image || null, remarks || null, now(), now()).run();
...
app.put('/:id', authMiddleware, async (c) => {
    try {
        ...
        await c.env.DB.prepare(`UPDATE vehicles SET plate_number = ?, brand = ?, model = ?, color = ?, year = ?, seats = ?, mileage = ?, daily_rate = ?, status = ?, is_new_energy = ?, vin = ?, engine_number = ?, license_image = ?, registration_image = ?, remarks = ?, updated_at = ? WHERE id = ?`).bind(plate_number, brand, model, color, year, seats, mileage, daily_rate, status, is_new_energy ? 1 : 0, vin, engine_number, license_image, registration_image, remarks, now(), id).run();
