...
app.get('/:id', authMiddleware, async (c) => {
    try {
        ...
        const user = await c.env.DB.prepare('SELECT id, username, name, role, phone, email, status, created_at FROM users WHERE id = ?').bind(id).first();
        if (!user) {
...
app.post('/', authMiddleware, async (c) => {
    try {
        ...
        await c.env.DB.prepare('INSERT INTO users (id, username, password, name, role, phone, email, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?)').bind(id, username, hashedPassword, name, role, phone || null, email || null, currentTime, currentTime).run();
        return c.json({
            success: true,
...
app.put('/:id', authMiddleware, async (c) => {
    try {
        ...
        await c.env.DB.prepare('UPDATE users SET name = ?, role = ?, phone = ?, email = ?, status = ?, updated_at = ? WHERE id = ?').bind(name, role, phone || null, email || null, status, now(), id).run();
