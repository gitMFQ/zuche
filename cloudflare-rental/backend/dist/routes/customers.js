...
app.get('/regular', authMiddleware, async (c) => {
    try {
        const data = await c.env.DB.prepare('SELECT * FROM customers WHERE is_regular = 1 ORDER BY name').all();
...
app.post('/', authMiddleware, async (c) => {
    try {
        ...
        const id = generateId();
        await c.env.DB.prepare(`INSERT INTO customers (id, name, phone, id_card, license_number, license_expiry, address, remarks, source_id, source_name, status, created_at, updated_at) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?)`).bind(id, name, phone, id_card || null, license_number || null, license_expiry || null, address || null, remarks || null, source_id || null, source_name || null, now(), now()).run();
...
app.put('/:id', authMiddleware, async (c) => {
    try {
        ...
        await c.env.DB.prepare(`UPDATE customers SET name = ?, phone = ?, id_card = ?, license_number = ?, license_expiry = ?, address = ?, remarks = ?, is_regular = ?, source_id = ?, source_name = ?, id_card_images = ?, license_images = ?, updated_at = ? WHERE id = ?`).bind(name, phone, id_card, license_number, license_expiry, address, remarks, is_regular ? 1 : 0, source_id, source_name, id_card_images, license_images, now(), id).run();
