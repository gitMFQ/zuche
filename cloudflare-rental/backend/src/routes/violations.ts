...
app.post('/', authMiddleware, async (c) => {
  try {
    ...
    await c.env.DB.prepare(
      `INSERT INTO violations (id, order_id, vehicle_id, plate_number, violation_date, type, description, fine, points, images, status, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?)`
    ).bind(id, order_id, vehicle_id, plate_number, violation_date, type, description, fine, points, images, now(), now()).run()
