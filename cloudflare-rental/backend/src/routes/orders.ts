...
app.put('/:id', authMiddleware, async (c) => {
  try {
    ...
    await c.env.DB.prepare(
      `UPDATE orders SET customer_name = ?, customer_phone = ?, vehicle_id = ?, plate_number = ?, start_date = ?, end_date = ?, total_amount = ?, discount_amount = ?, actual_amount = ?, deposit = ?, service_type = ?, deposit_waived = ?, deposit_waived_expiry = ?, net_amount = ?, remarks = ?, updated_at = ? WHERE id = ?`
    ).bind(customer_name, customer_phone, vehicle_id, plate_number, start_date, end_date, total_amount, discount_amount, actual_amount, deposit, service_type, deposit_waived ? 1 : 0, deposit_waived_expiry, net_amount, remarks, now(), id).run()
