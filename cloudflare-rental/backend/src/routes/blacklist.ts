...
app.post('/', authMiddleware, async (c) => {
  try {
    ...
    await c.env.DB.prepare(
      `INSERT INTO blacklist (id, customer_id, name, phone, id_card, reason, order_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
    ).bind(id, customer_id, name, phone, id_card, reason, order_id, now()).run()
