...
#### 车辆详情弹窗组件
- **新增文件**:
  - `frontend/src/components/VehicleDetailDialog.vue` - 车辆详情弹窗组件
- **修改文件**:
  - `frontend/src/components/VehiclesTab.vue` - 集成详情弹窗
- **变更内容**:
  - 新增车辆详情弹窗组件，显示车牌号、品牌、型号、颜色、年份、座位数、里程、日租金、押金、车架号、发动机号、状态、备注
  - 支持新能源车标识显示
  - 支持行驶证、登记证书图片展示和预览
  - 点击"查看完整信息"跳转到订单页面
  - 车辆列表点击查看按钮打开详情弹窗
...
- **变更内容**:
  - 甘特图数据返回完整的车辆信息（品牌、型号、颜色、年份、座位数、里程、日租金、押金、行驶证、登记证书等）
  - 返回车辆状态、备注等详细信息
...
#### 代码清理优化
- **修改文件**:
  - `frontend/src/router/index.ts` - 路由守卫参数优化
  - `AGENTS.md` - 项目文档精简
  - `QWEN.md` - 文档更新
